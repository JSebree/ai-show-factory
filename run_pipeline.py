...
def publish_episode(mp3_path: str, metadata: dict) -> str:
    ...
    Returns the public URL (bucket must allow public-read).
    """
...
if __name__ == "__main__":
    ...
    # 3) Flatten dialogue or full text into a single string
    raw_dialogue = script.get("dialogue") \
                or script.get("full_script") \
                or script.get("text") \
                or ""
    ...
    if isinstance(raw_dialogue, list):
        ...
        for elem in raw_dialogue:
            if isinstance(elem, str):
                ...
                # prefer 'text' or fallback to any string field
    ...
    if not full_text:
        raise RuntimeError("LLM returned empty dialogue/full text block.")

    # 4) Split into lines and synthesize per speaker turn
    lines = [ln for ln in full_text.splitlines() if ln.strip()]
    segments = []
    for idx, line in enumerate(lines, start=1):
        # Assign voice based on prefix
        ...
        tts(text=line, out_path=seg_file, voice_id=vid)
